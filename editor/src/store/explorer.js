...
class Node {
  ...
  append(items) {
    ...
    children.forEach((i) => {
      if (i.type === 'folder') {
        this.folders.push(new Node(i, [], true));
      } else if (i.type === 'file') {
        this.files.push(new Node(i, [], false));
      }
    });
    this.sortChildren();
  }
...
export default {
  ...
  mutations: {
    ...
    setChildrenLoading(state, { id, loading }) {
      Vue.set(state.loadingNodes, id, loading);
    },
  },
  getters: {
    nodeStructure: (state, getters, rootState, rootGetters) => {
      ...
      // });
      // debugger;
      rootGetters['file/Files'].forEach((item) => {
        if (item.type === 'folder') {
          nodeMap[item.id] = new Node(item, [], true);
      ...
      Object.entries(nodeMap).forEach(([, node]) => {
        ...
        if (!parentNode || !parentNode.isFolder) {
          ...
          if (!parentNode && node.item.parentId) {
            return;
          }
          parentNode = rootNode;
        }
