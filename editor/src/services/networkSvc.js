...
export default {
  ...
  async getServerConf() {
    ...
    if (!store.state.offline && !isConfLoading && !isConfLoaded) {
      try {
        isConfLoading = true;
        const res = await this.request({ url: 'api/conf' });
  ...
  async startOauth2(url, params = {}, silent = false, reattempt = false) {
    try {
      ...
      const authorizeUrl = utils.addQueryParams(url, {
        ...params,
        state,
        // redirect_uri: constants.oauth2RedirectUri, // 去除此设置，github会使用应用中设置的返回地址
      });
      // debugger;
      let iframeElt;
      let wnd;
      ...
      try {
        return await new Promise((resolve, reject) => {
          ...
          msgHandler = (event) => {
            // debugger;
            // alert(0);
            if (event.source === wnd && (true || event.origin === constants.origin)) {
