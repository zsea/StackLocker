  ...
  <div class="layout" :class="{'layout--revision': revisionContent}">
    <div class="layout__panel flex flex--row" :class="{'flex--end': styles.showSideBar}">
      ...
      <div class="layout__panel flex flex--column" tour-step-anchor="welcome,end" :style="{width: styles.innerWidth + 'px'}">
        ...
        <div v-if="loading" style="height: 100%;"><spin text="加载中..."></spin></div>
        <empty-screen v-if="!current.id"></empty-screen>
        <div v-show="current.id&&!loading" class="layout__panel flex flex--row" :style="{height: styles.innerHeight + 'px'}">
        ...
        <div class="layout__panel layout__panel--status-bar" v-show="styles.showStatusBar&&current.id" :style="{height: constants.statusBarHeight + 'px'}">
...
export default {
  ...
  mounted() {
    ...
    // 编辑器挂载成功，在此处监听编辑器的内容修改事件，更建议在内容store中修改(content.js)
    // cloudSvc.watchContent(editorElt);
