...
module.exports = (app) => {
  ...
  app.get('/api/conf', (req, res) => res.send(conf.publicValues));
  ...
  app.use('/api', proxy({ target: `http://127.0.0.1:8788` }));
  //app.use('/api/account',proxy({ target: `http://127.0.0.1:8788/api/account` }));

  // app.use("/",
  //   proxy([`/api/disk`], { target: `http://127.0.0.1:8788/api/disk/` }));
  // app.use("/",
  //   proxy([`/api/account`], { target: `http://127.0.0.1:8788/api/account/` }));
